...
impl SoftCommands {
    fn create_server_entry(guild_id: &str) -> Vec<SoftCommands> {
    ...
    pub fn get_soft_commands_sql(guild_id: &str) -> Vec<SoftCommands> { 
        ...
        match server_table_exists {
            Some(_val) => {//table exists for this server
                return sqlpool::return_sql_connection()
                    .query_map(
            ...
            None => return Self::create_server_entry(guild_id), //table doesn't exist yet
        }; 
        //soft_commands
    }
}
